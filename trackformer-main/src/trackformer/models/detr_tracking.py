...
class DETRTrackingBase(nn.Module):
    ...
    def add_track_queries_to_targets(self, targets, prev_indices, prev_out, add_false_pos=True):
        ...
        for i, (target, prev_ind) in enumerate(zip(targets, prev_indices)):
            ...
            if add_false_pos:
                ##########################################################################
                # A problem occurred with colab and the tensors were on different devices

                # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
                # print(f"Used device: {device}")


                # Check and move the indices to the correct device
                prev_out_ind = prev_out_ind.to(prev_out['pred_boxes'].device)
                target_ind_matching = target_ind_matching.to(prev_out['pred_boxes'].device)
