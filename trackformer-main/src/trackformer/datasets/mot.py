...
def build_mot_custom_dataset(image_set, args):
    root = Path("/content/drive/MyDrive/trackformer-main/data/custom_dataset")
    assert root.exists(), f'Provided path for the custom MOT dataset does not exist: {root}'

    if image_set == 'train':
        img_folder = root / 'train_sequences'
        ann_file = root / 'annotations' / 'train.json'
        prev_frame_rnd_augs = getattr(args, 'custom_prev_frame_rnd_augs', 0.5)
    elif image_set == 'val':
        img_folder = root / 'val'
        ann_file = root / 'annotations' / 'val.json'
        prev_frame_rnd_augs = 0.0
    elif image_set == 'test_sequences':
        img_folder = root / 'test_sequences'
        ann_file = root / 'annotations' / 'val.json'
    else:
        raise ValueError(f"Unknown image set: {image_set}")

    transforms, norm_transforms = make_coco_transforms(
        image_set, getattr(args, 'img_transform', None), getattr(args, 'overflow_boxes', False))
