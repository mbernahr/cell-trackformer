...
def evaluate(model, criterion, postprocessors, data_loader, device,
    ...
    for i, (samples, targets) in enumerate(metric_logger.log_every(data_loader, 'Test:')):
        ...
        if visualizers and (i == 0 or not i % 5):
            #print(('TRYING TO VISUALZE STHNG'))
    ...
    if args.tracking and args.tracking_eval:
        ...
        #print(f'DATALOADER.DATASET: {data_loader.dataset}')
        ...
        #print(f'OBJ_DETECTOR_MODEL: {obj_detector_model}')
        ...
        #print(f'CONFIG_UPDATES: {config_updates}')
        run = ex.run(config_updates=config_updates)
        print(f'RUN_RESULT: {run.result}')
